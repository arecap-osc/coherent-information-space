...
"""
viz_subgraphs.py (Deliverable B)

Draws 3x2 schematics:
- Upstream (CornerParity, SideParity)
- Downstream (CornerParity, SideParity)
- Superposition (CornerParity, SideParity)

Uses your existing:
  coherent_space_py.model.enums
  coherent_space_py.model.topology_rules

Includes safe fallbacks (no arrows) if those modules don't exist.
"""
...
# Topology rules fallbacks
try:
    from coherent_space_py.model.topology_rules import get_upstream_connections, get_downstream_connections
except Exception:
    def get_upstream_connections(source_type, vd):
        return []
    def get_downstream_connections(source_type, vd):
        return []
...
def draw_layer(ax, *, mode: str, vd, draw_background: bool = True, draw_labels: bool = True):
    ...
    def draw_arrows(node_map, get_targets_fn):
        for s_type, s_pos in node_map.items():
            targets = get_targets_fn(s_type, vd)
