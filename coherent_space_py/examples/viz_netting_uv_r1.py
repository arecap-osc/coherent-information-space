...
def draw_cell(
    ...
    def draw_arrows(node_map, get_targets_fn, *, target_filter=None):
        for s_type, s_pos in node_map.items():
            targets = list(get_targets_fn(s_type, vd_cell))
    ...
    # Downstream arrows: apply only your exclusion list, parity varies per cell via vd_cell
    if down_nodes:
        draw_arrows(down_nodes, get_downstream_connections, target_filter=downstream_filter_targets)
