...
import org.hkrdi.eden.ggm.vaadin.console.microservice.coherentspace.road.media.layer.RoadSelectedNeighbourNodesMediaLayer;
import org.hkrdi.eden.ggm.vaadin.console.microservice.coherentspace.road.media.layer.RoadStartNodeMediaLayer;
import org.hkrdi.eden.ggm.vaadin.console.microservice.coherentspace.road.media.manual.RoadManualRoadLayerManager;
...
@SpringComponent
@UIScope
public class RoadManualRoadStateSelectionManager extends RoadStateSelectionManager{
	@Autowired
	private RoadManualRoadLayerManager layerManager;
	
	@Autowired
    private CoherentSpaceService coherentSpaceService;
	
	public void mapFormFieldsToCurrentState(NodeBean selectedNode, NodeBean toNode, List<NodeBean> manualNodes, List<ClusterBean> excludeClusters, List<NodeBean> road) {
    	if (getCurrentNetwork()!=null) {
	    	...
	    	if (manualNodes!=null) {
		    	if (manualNodes.size()>0) {
		    		currentState().setStartNode(manualNodes.get(0));
		    		
		    		//Neighbors
		    		NodeBean lastNode = manualNodes.get(manualNodes.size()-1);
		    		currentState().setSelectedNeighborsNodes(coherentSpaceService.findToNodeNeighbors(lastNode));
		    	}
		    	
		    	if (manualNodes.size()>1) {
		    		currentState().setEndNode(manualNodes.get(manualNodes.size()-1));
		    	}

		    	//remove first&last
		    	currentState().getIncludeNodes().addAll(manualNodes.stream().limit(manualNodes.size()-1).skip(1).collect(Collectors.toList()));
		    	
	    		currentState().getRoad().addAll(manualNodes);
	    	}    	
...
//	    	}
	...
	public void mapCurrentStateToMediaLayers() {
		if (getCurrentNetwork()!=null) {
	    	...
	    	//
	    	Optional<RoadSelectedNeighbourNodesMediaLayer> optionaNeighborLayer = getLayerByNetworkAndType(layerManager, getCurrentNetwork(), RoadSelectedNeighbourNodesMediaLayer.class);
	    	if (optionaNeighborLayer.isPresent()) {
	    		RoadSelectedNeighbourNodesMediaLayer neighborLayer = optionaNeighborLayer.get();
		    	if (neighborLayer.getSelectedIndex()!=null && (
		    			(neighborLayer.getSelectedIndex().size() != 0 && currentState().getSelectedNeighborsNodes().size() == 0) ||
		    			(neighborLayer.getSelectedIndex().size() == 0 && currentState().getSelectedNeighborsNodes().size() != 0) ||
		    			(neighborLayer.getSelectedIndex().size() != 0 && currentState().getSelectedNeighborsNodes().size() != 0 && 
		    				!neighborLayer.getSelectedIndex().equals(currentState().getSelectedNeighborsNodes().stream().map(node -> node.getAddressIndex()).collect(Collectors.toList()))))) {
		    			neighborLayer.setSelectedIndex(currentState().getSelectedNeighborsNodes().stream().map(node -> node.getAddressIndex()).collect(Collectors.toList()));
		    		neighborLayer.setNeedRefresh(true);
		    	}
			}
...
//	}
	...
	@Override
	public RoadManualRoadLayerManager getLayerManager() {
		return layerManager;
	}
	
	public void setLayerManager(RoadManualRoadLayerManager layerManager) {
